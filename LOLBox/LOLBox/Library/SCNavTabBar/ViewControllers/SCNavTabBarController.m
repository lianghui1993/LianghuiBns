...
- (void)viewInit
{
    ...
    // 顶部横条滚动视图设置
    _navTabBar = [[SCNavTabBar alloc] initWithFrame:CGRectMake(DOT_COORDINATE, DOT_COORDINATE + 20, SCREEN_WIDTH, NAV_TAB_BAR_HEIGHT) showArrowButton:_showArrowButton];
    ...
    // 里面内容视图设置
    _mainView = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE,_navTabBar.frame.origin.y + _navTabBar.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT - _navTabBar.frame.origin.y - _navTabBar.frame.size.height - STATUS_BAR_HEIGHT - NAVIGATION_BAR_HEIGHT + 20)];
...
- (void)viewConfig
{
    ...
    [_subViewControllers enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop){
        ...
        viewController.view.frame = CGRectMake(idx * SCREEN_WIDTH, DOT_COORDINATE + 20, SCREEN_WIDTH, _mainView.frame.size.height);
